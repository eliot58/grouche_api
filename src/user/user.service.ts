...
export class UserService {
    ...
    async checkBurnAll() {
        ...
        for (const nftItem of uncheckedItems) {
            try {
                const bounceNftAddress = Address.parse(nftItem.address).toString({ bounceable: true });
                this.logger.log(`Checking NFT item ${bounceNftAddress}`);

                const history = await client.nft.getNftHistoryById(Address.parse(nftItem.address), { limit: 10 }, {});
                const eventId = history.events?.[0]?.eventId;
                if (!eventId) {
                    this.logger.warn(`No NFT events found for ${bounceNftAddress}, skipping`);
                    continue;
                }
                ...
                if (!action) {
                    this.logger.warn(`NftItemTransfer action not found in event for ${bounceNftAddress}, skipping`);
                    continue;
                }
                ...
                if (recipient !== burnRecipient) {
                    this.logger.log(`NFT ${bounceNftAddress} recipient is not burn address, skipping`);
                    continue;
                }

                let user = await this.prisma.user.findUnique({ where: { wallet: sender } });
                if (!user) {
                    this.logger.log(`User ${sender} not found, creating new user`);
                    user = await this.prisma.user.create({ data: { wallet: sender!, limit: 0 } });
                }

                const match = nftItem.content.match(/^(\d+)\.json$/);
                const pointsToAdd = match ? parseInt(match[1], 10) : 0;
                if (pointsToAdd <= 0) {
                    this.logger.warn(`Invalid NFT content format or zero points for ${bounceNftAddress}, skipping`);
                    continue;
                }
                ...
                this.logger.log(`Added ${pointsToAdd} points to user ${sender} for NFT ${bounceNftAddress}`);

            } catch (error) {
                this.logger.error(`Error processing NFT item ${nftItem.address}: ${error.message}`, error.stack);
            }
        }
